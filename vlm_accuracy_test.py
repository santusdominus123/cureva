...
class VisualizationGenerator:
    ...
    def _create_overview_dashboard(self, df_results: pd.DataFrame, df_scores: pd.DataFrame):
        ...
        fig.suptitle('🧪 VLM ACCURACY TEST - OVERVIEW DASHBOARD',
        ...
        if 'success' in df_results.columns:
            ...
            ax1.pie(success_counts, labels=['✅ Success', '❌ Failed'],
                   autopct='%1.1f%%', colors=colors, startangle=90,
                   textprops={'fontsize': 10, 'weight': 'bold'})
            ax1.set_title('Success Rate', fontsize=12, fontweight='bold', pad=10)

        # 2. Processing Time Box Plot
        ax2 = plt.subplot(3, 4, 2)
        if 'processing_time' in df_results.columns and not df_results.empty:
            bp = ax2.boxplot(df_results['processing_time'],
                            patch_artist=True, vert=True)
            bp['boxes'][0].set_facecolor(COLORS['primary'])
            ax2.set_ylabel('Seconds', fontweight='bold')
            ax2.set_title('⏱️ Processing Time Distribution',
                         fontsize=12, fontweight='bold', pad=10)
            ax2.grid(True, alpha=0.3)

        # 3. Confidence Distribution
        ax3 = plt.subplot(3, 4, 3)
        if 'confidence' in df_results.columns and not df_results.empty:
            ax3.hist(df_results['confidence'], bins=15,
                    color=COLORS['info'], alpha=0.7, edgecolor='black')
            mean_conf = df_results['confidence'].mean()
            ax3.axvline(mean_conf, color=COLORS['danger'],
        ...
        if not df_scores.empty and 'overall_score' in df_scores.columns:
            ...
            self._create_gauge(ax4, overall_mean, 'Overall Score')

        # 5. Score Components Bar Chart
        ...
        if not df_scores.empty:
            ...
            if available_cols:
                ...
                labels = ['Correctness', 'Completeness', 'Relevance', 'Detail', 'Language'][:len(means)]
                bars = ax5.barh(labels, means, color=[COLORS['primary'], COLORS['success'],
                                                       COLORS['warning'], COLORS['purple'],
                                                       COLORS['teal']][:len(means)])
                ax5.set_xlabel('Score (0-10)', fontweight='bold')
                ax5.set_title('📊 Score Components', fontsize=12, fontweight='bold', pad=10)
                ax5.set_xlim(0, 10)
                ax5.grid(True, alpha=0.3, axis='x')

                # Add value labels
                for i, (bar, value) in enumerate(zip(bars, means)):
                    ax5.text(value + 0.2, i, f'{value:.2f}',
                            va='center', fontweight='bold')

        # 6. F1 Score by Test Type
        ax6 = plt.subplot(3, 4, 6)
        if not df_scores.empty and 'test_type' in df_scores.columns and 'f1_score' in df_scores.columns:
            test_f1 = df_scores.groupby('test_type')['f1_score'].mean().sort_values(ascending=False)
            if not test_f1.empty:
                bars = ax6.bar(range(len(test_f1)), test_f1.values,
                              color=COLORS['success'], alpha=0.7, edgecolor='black')
                ax6.set_xticks(range(len(test_f1)))
                ax6.set_xticklabels(test_f1.index, rotation=45, ha='right')
                ax6.set_ylabel('F1 Score %', fontweight='bold')
                ax6.set_title('🎯 F1 Score by Test Type', fontsize=12, fontweight='bold', pad=10)
                ax6.grid(True, alpha=0.3, axis='y')

                # Add value labels
                for bar, value in zip(bars, test_f1.values):
                    ax6.text(bar.get_x() + bar.get_width() / 2, value + 1,
                            f'{value:.1f}%', ha='center', fontweight='bold')

        # 7. Processing Time by Test Type
        ax7 = plt.subplot(3, 4, 7)
        if 'test_type' in df_results.columns and 'processing_time' in df_results.columns and not df_results.empty:
            test_time = df_results.groupby('test_type')['processing_time'].mean().sort_values()
            if not test_time.empty:
                bars = ax7.barh(test_time.index, test_time.values,
                               color=COLORS['warning'], alpha=0.7, edgecolor='black')
                ax7.set_xlabel('Seconds', fontweight='bold')
                ax7.set_title('⏱️ Avg. Time by Test Type', fontsize=12, fontweight='bold', pad=10)
                ax7.grid(True, alpha=0.3, axis='x')

                for bar, value in zip(bars, test_time.values):
                    ax7.text(value + 0.05, bar.get_y() + bar.get_height() / 2,
                            f'{value:.2f}s', va='center', fontweight='bold')

        # 8. Precision vs Recall Scatter
        ax8 = plt.subplot(3, 4, 8)
        if not df_scores.empty and 'precision' in df_scores.columns and 'recall' in df_scores.columns:
            scatter = ax8.scatter(df_scores['precision'], df_scores['recall'],
                                 c=df_scores['f1_score'], cmap='RdYlGn',
                                 s=100, alpha=0.6, edgecolors='black')
            ax8.plot([0, 100], [0, 100], 'k--', alpha=0.3, label='Perfect Balance')
            ax8.set_xlabel('Precision %', fontweight='bold')
            ax8.set_ylabel('Recall %', fontweight='bold')
            ax8.set_title('🎯 Precision vs Recall', fontsize=12, fontweight='bold', pad=10)
            ax8.legend()
            ax8.grid(True, alpha=0.3)
            plt.colorbar(scatter, ax=ax8, label='F1 Score')

        # 9-12: Additional metrics if available
        # Word Count Distribution
        ax9 = plt.subplot(3, 4, 9)
        if 'word_count' in df_results.columns and not df_results.empty:
            ax9.hist(df_results['word_count'], bins=15,
                    color=COLORS['purple'], alpha=0.7, edgecolor='black')
            ax9.set_xlabel('Word Count', fontweight='bold')
            ax9.set_ylabel('Frequency', fontweight='bold')
            ax9.set_title('📝 Response Length', fontsize=12, fontweight='bold', pad=10)
        ...
        if not df_scores.empty:
            ...
            if available_cols:
                ...
                ax10.plot(angles, means, 'o-', linewidth=2, color=COLORS['primary'])
                ax10.fill(angles, means, alpha=0.25, color=COLORS['primary'])
                ax10.set_xticks(angles[:-1])
                ax10.set_xticklabels(labels, size=8)
                ax10.set_ylim(0, 10)
                ax10.set_title('⭐ Score Radar', fontsize=12, fontweight='bold', pad=20)
                ax10.grid(True)
        ...
        if not df_results.empty:
            summary_data.append(['Total Tests', len(df_results)])
            if 'success' in df_results.columns:
                summary_data.append(['Success Rate', f"{df_results['success'].mean()*100:.1f}%"])
            if 'processing_time' in df_results.columns:
                summary_data.append(['Avg. Time', f"{df_results['processing_time'].mean():.2f}s"])
            if 'confidence' in df_results.columns:
                summary_data.append(['Avg. Confidence', f"{df_results['confidence'].mean():.1f}%"])

        if not df_scores.empty and 'overall_score' in df_scores.columns:
            summary_data.append(['Avg. Score', f"{df_scores['overall_score'].mean():.2f}/10"])
            summary_data.append(['Grade', self._get_grade(df_scores['overall_score'].mean())])
        ...
        if summary_data:
            ...
            ax11.set_title('📋 Summary Statistics', fontsize=12, fontweight='bold', pad=10)

        # Grade Display
        ax12 = plt.subplot(3, 4, 12)
        ax12.axis('off')
        if not df_scores.empty and 'overall_score' in df_scores.columns:
            grade = self._get_grade(df_scores['overall_score'].mean())
            grade_color = self._get_grade_color(grade)

            ax12.text(0.5, 0.6, 'FINAL GRADE', ha='center', fontsize=16, fontweight='bold')
            ax12.text(0.5, 0.3, grade, ha='center', fontsize=60, fontweight='bold',
                     color=grade_color)
            ax12.set_xlim(0, 1)
            ax12.set_ylim(0, 1)
        ...
        print(f"  {EMOJI['check']} Overview dashboard saved")
